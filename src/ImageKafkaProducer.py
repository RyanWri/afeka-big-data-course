...
class ImageKafkaProducer(threading.Thread):
    ...
    def fetch_image(self):
        ...
        try:
            response = requests.get(f"https://picsum.photos/{self.image_size}?grayscale", timeout=3)
    ...
    def delivery_report(self, err, msg):
        ...
        if err:
            ...
            print(f"Image sent to {msg.topic()} [{msg.partition()}] at offset {msg.offset()}")
    ...
    def run(self):
        ...
        while self.running:
            self.produce_image()
            time.sleep(self.produce_timer)  # Fetch an image every few seconds (5 by default).
